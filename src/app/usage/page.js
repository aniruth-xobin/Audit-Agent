...
export default function UsagePage() { 
  ...
  if (!data || data.error) {
    return <div className="p-8 text-center text-xs font-mono text-[var(--chart-red)]">Error loading data.</div>;
  }
    ...
    <div className="flex flex-col gap-6 max-w-[1400px] mx-auto pb-12 font-mono sm:font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-5 flex flex-col h-[180px]">
          ...
          <div className="flex-1 flex items-center justify-center">
             <div className="flex flex-col items-center">
                <span className="text-[40px] font-medium text-[var(--chart-cyan)] tracking-tight">{data.totalAudits}</span>
                <span className="text-xs text-[var(--text-muted)] mt-1">Processed</span>
             </div>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-5 flex flex-col h-[180px]">
          ...
          <div className="flex-1 flex items-center justify-center">
             <div className="flex flex-col items-center">
                <span className="text-[40px] font-medium text-[var(--chart-red)] tracking-tight">{data.flaggedPercent}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] h-[300px] flex flex-col">
          ...
          <div className="flex-1 w-full pb-4">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.latencyData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                <CartesianGrid stroke="var(--border-color)" vertical={false} horizontal={true} />
