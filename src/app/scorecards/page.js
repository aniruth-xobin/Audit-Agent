                  ...
                  <div className="flex flex-col gap-4">
                    {getDeductions(activeSession).map((deduction, i) => (
                      <div key={i} className="flex flex-col gap-3 p-4 bg-[var(--bg-secondary)]/40 border border-[var(--border-strong)] rounded-lg">
                        ...
                        {/* Header row: turn badge + type label + metric pill */}
                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--border-strong)] pb-2">
                          <div className="flex items-center gap-3">
                            {deduction.turn_number ? (
                              ...
                              <button
                                onClick={() => router.push(`/transcripts?id=${activeSession.id}&time=${deduction.time}`)}
                                className="font-mono text-xs font-bold text-[var(--chart-cyan)] bg-[var(--chart-cyan)]/10 hover:bg-[var(--chart-cyan)]/25 px-2.5 py-1 rounded-md border border-[var(--chart-cyan)]/20 hover:border-[var(--chart-cyan)]/50 transition-all cursor-pointer underline-offset-2 hover:underline whitespace-nowrap inline-flex items-center gap-1"
                                title="Jump to transcript"
                              >
                                {deduction.time || 'General'} &#x2197;
                              </button>
                            )}
                          ...
                          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border"
                            style={{
                              background: deduction.tool_status === 'success'
                                ...
                                : 'color-mix(in srgb, var(--chart-red) 8%, transparent)',
                              borderColor: deduction.tool_status === 'success'
                                ? 'color-mix(in srgb, var(--chart-green) 25%, transparent)'
                        ...
                        {/* Hallucination recovery block */}
                        {deduction.type === 'HALLUCINATION' && (
                          <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg border"
                            style={{
                              background: deduction.recovered
                                ? 'color-mix(in srgb, var(--chart-green) 8%, transparent)'
                                : 'color-mix(in srgb, var(--chart-red) 8%, transparent)',
                              borderColor: deduction.recovered
                                ? 'color-mix(in srgb, var(--chart-green) 25%, transparent)'
                                : 'color-mix(in srgb, var(--chart-red) 25%, transparent)',
                            }}
                          >
                            <span className="text-base leading-none shrink-0 mt-0.5"
                              style={{ color: deduction.recovered ? 'var(--chart-green)' : 'var(--chart-red)' }}>
                              {deduction.recovered ? '↩' : '⚠'}
                            </span>
                            <div className="flex flex-col gap-0.5">
                              <span className="text-[11px] font-bold uppercase tracking-widest"
                                style={{ color: deduction.recovered ? 'var(--chart-green)' : 'var(--chart-red)' }}>
                                {deduction.recovered ? 'Recovered' : 'Not Recovered'}
                              </span>
                              <span className="text-xs text-[var(--text-muted)]">
                                {deduction.recovered
                                  ? `Agent self-corrected at turn ${deduction.recovery_turn} (${deduction.recovery_time}) — took ${deduction.recovery_turns_taken} turn${deduction.recovery_turns_taken !== 1 ? 's' : ''}.`
                                  : 'Agent never corrected this hallucination during the session.'}
                              </span>
                            </div>
                          </div>
                        )}
                        ...
                        {deduction.insight && (
                          <div className="flex gap-3 p-3.5 bg-[var(--bg-card)] border border-[var(--border-strong)] rounded-md text-xs mt-1">
                            ...
                            <div>
                              <span className="font-semibold text-[var(--text-primary)] mb-1.5 block uppercase tracking-wider text-[10px]">Insight</span>
